...
def init_hidden(x, hidden_size: int):
    ...
    """
    hidden = Variable(torch.zeros(1, x.size(0), hidden_size))
...
def init_zeros(*sizes):
    zeros = Variable(torch.zeros(*sizes))
    if torch.cuda.is_available():
        zeros = zeros.cuda()
...
class Encoder(nn.Module):
    ...
    def forward(self, input_data):
        ...
        attn_w = init_zeros(input_data.size(0), self.T - 1, self.input_size)
        ...
        for t in range(self.T - 1):
            ...
            attn_w[:, t, :] = attn_weights
        return input_weighted, input_encoded.to(device), attn_w
